...
\item To avoid discontinuity at the boundary of the patches, we use a frame around the actual patch to capture more information. This frame is not filled in by the reconstructed patch from the Fourier step. (((IMAGE))) The frames of adjacent patches are overlapping, if the frame size is set to a value larger than zero.\end{itemize}
((( TODO )))
